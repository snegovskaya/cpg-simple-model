...
def Iperiod_impulse(t: float, impulseAmpl = 5, impulseLength = 5, period = 10, tStart = 1, tFinish = 250): 
    if t > tFinish: 
        I = 0
...
test_muscle = Muscle(CN0, F0, input = (test_neuron.v - v0)/80) # t=t[0] 
# Вот это вот магическое /80 * 20 — это попытка в «ну у Wilson вроде как 
# u нормировалось на максимальное значение, но это не точно»
# (Конкретно сейчас это попытка отнормировать типа по Ding)
